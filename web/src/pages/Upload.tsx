  ...
  return (
    <div className="mb-6">
      ...
      {searchResult && searchResult.local.length > 0 && (
        <div className="mb-3">
          <p className="text-xs text-ink-faint mb-1.5">Already in library</p>
          <div className="space-y-1">
            {searchResult.local.map((hit) => (
              <div key={hit.id} className="flex items-center gap-2 px-3 py-2 rounded-lg bg-surface-2 border border-border text-sm">
                <Music size={13} className="text-ink-faint flex-shrink-0" />
      ...
      {searchResult && searchResult.youtube.length > 0 && (
        <div className="space-y-1.5 mb-3">
          <p className="text-xs text-ink-faint mb-1.5">YouTube</p>
          {searchResult.youtube.map((r) => {
            const added = addedIds.has(r.id)
            return (
              <div key={r.id} className="flex items-center gap-3 p-2 rounded-lg bg-surface-2 border border-border">
                {r.thumbnail ? (
                  <img src={r.thumbnail} alt="" className="w-16 h-10 rounded object-cover flex-shrink-0 bg-surface-2" />
                ) : (
                  <div className="w-16 h-10 rounded bg-surface-2 flex items-center justify-center flex-shrink-0">
                    <FileVideo size={14} className="text-ink-faint" />
                  </div>
                )}
      ...
      {lastQuery && searchResult && !hasResults && (
        <p className="text-xs text-ink-faint text-center py-4">No results found</p>
      )}

      {jobs.length > 0 && (
        <div className="space-y-1.5">
          {jobs.map((job) => (
            <div key={job.id} className="p-3 rounded-lg bg-surface-2 border border-border">
              <div className="flex items-center gap-2 mb-1">
                ...
                {job.status === 'done' && job.total_items && (
                  <span className="text-xs text-ink-secondary">{job.completed_items} tracks</span>
                )}
              </div>
      ...
      <div className="flex-1 min-w-0">
        {editing ? (
          ...
          <>
            ...
            <p className="text-xs text-ink-faint truncate font-mono">{file.filename}</p>
          </>
        )}
      </div>
    ...
    <div className="p-4 sm:p-6 max-w-2xl">
      ...
      <div
        className={`border-2 border-dashed rounded-xl p-10 text-center cursor-pointer transition-colors mb-6 ${
          dragging ? 'border-accent bg-accent/5' : 'border-border-strong hover:border-ink-faint'
        }`}
      ...
      <div>
        <h3 className="text-sm font-semibold text-ink-secondary uppercase tracking-wider mb-2">My Files</h3>
        {isLoading ? (
          <div className="space-y-1.5">
