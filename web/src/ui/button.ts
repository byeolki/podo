...
export const btn = {
  ...
  primary: `${base} bg-accent text-white shadow-raised hover:bg-accent-hover`,
  /** Everything else with a box around it. */
  secondary: `${base} bg-surface-2 border border-border hover:bg-surface-3 hover:border-border-strong`,
  /** No box until you touch it — toolbars, rows, anywhere a border would add noise. */
  ghost: `${base} text-ink-secondary hover:text-ink-primary hover:bg-white/[0.06]`,
  /** Destructive. Tinted rather than filled, so it doesn't compete with primary. */
  danger: `${base} bg-danger/10 text-danger border border-danger/30 hover:bg-danger/20`,
} as const
